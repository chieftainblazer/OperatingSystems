...
int main (int argc, char * argv[])
{
  ...
  */
  ...
  producer->sumtotal = 0, producer->multtotal = 0, producer->matrixtotal = 0;

  ProdConsStats * consumer = (ProdConsStats *) malloc(sizeof(ProdConsStats));
  consumer->sumtotal = 0, consumer->multtotal = 0, consumer->matrixtotal = 0;

  for (int i = 0; i < NUMWORK; i++) {
    pthread_create(&pr[i], NULL, prod_worker, producer);
    pthread_create(&co[i], NULL, cons_worker, consumer);
  }

  for (int i = 0; i < NUMWORK; i++) {
    pthread_join(pr[i],  (void *) &producer);
    pthread_join(co[i], (void *) &consumer);
  }
